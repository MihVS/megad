...
* [**_Блоки расширения I2C._**](#блоки-расширения-i2c)
  ...
  * [_TouchOn 0-10 Вольт_](#touchon-0-10-вольт)
* [**_Блоки расширения 1-WIRE._**](#блоки-расширения-1-wire)
...
Выход для адресных лент WS281X не имеет обратной связи с НА.

Конфигурация порта для НА осуществляется через поле Title.

Формат: имя сенсора/тип/связанный порт
* Имя устройства может быть любым, главное ограничение количество символов
* Тип указывается для соответствия цветовой схемы в НА.
  * rgb
  * rbg
  * grb
  * gbr
  * brg (для лент WS2811)
  * bgr
* Связанный порт - это релейный порт через который подключено питание 
ленты. Это нужно чтобы снять питание с ленты. (Защита от перегрева и 
потребления энергии в режиме ожидания команды лентой). 
Это необязательный параметр, ленту можно подключать напрямую к источнику питания.

> [!WARNING]
> Для связанного порта установлена задержка (0.5 с) отправки команды на ленту 
> после включения релейного выхода. Пауза нужна для включения блока питания ленты.

Примеры:
* `Полка/brg` --> Имя "Полка" / Тип brg / лента подключена к питанию постоянно
* `Детская//7` --> Имя "Детская" / Тип rgb / лента подключена к питанию через реле выхода 7
* `Штора/rgb/9` --> Имя "Штора" / Тип rgb / лента подключена к питанию через реле выхода 9
...
### TouchOn 0-10 Вольт
Краткая информация по модулю [тут](https://wiki.touchon.ru/index.php/Модуль_0-10V).

Данный модуль расширяет количество диммируемых выходов.

## Блоки расширения 1-WIRE
...
Эти модули имеют 2 выхода. Их можно подключать на один порт шиной.
Информация по настройке порта в [официальной документации](https://ab-log.ru/smart-house/ethernet/megad-2561#conf-out-ds).

> [!WARNING]
> Контроллер не шлёт информацию об изменении состояния такого выхода, 
> обновление данных происходит примерно раз в минуту.
 
Настройка поля `Title` осуществляется аналогично [релейным выходам](#релейные-выходы).
