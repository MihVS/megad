    ...
    async def async_step_user(self, user_input=None):
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema(
                {
                    vol.Required(schema="ip", default=DEFAULT_IP): str,
                    vol.Required(
                        schema="password", default=DEFAULT_PASSWORD): str
                }
            ),
            errors=errors
        )
        ...
        if user_input is not None:
            ...
            try:
                ...
                if not errors:
                    ...
                    _LOGGER.debug(f'options stap init: {self.data}')
                return await self.async_step_get_config()
            except InvalidIpAddressExist:
                _LOGGER.error(f'IP адрес уже используется в интеграции: {ip}')
                errors['base'] = 'ip_exist'
            ...
            data_schema=vol.Schema(
                {
                    vol.Required(
                        schema="name_file",
                        default=self.data["file_path"].split("/")[-1]
                    ): str,
                    vol.Optional(schema="return_main_menu"): bool
                }
            ),
